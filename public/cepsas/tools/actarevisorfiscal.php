...
$pdf->Ln(5);
$siguienteaño = $año + 1;
$txt = 'ACTA DE CIERRE DE ESCRUTINIO ELECCIÓN REVISOR FISCAL '.$año.'-'.$siguienteaño;  
$pdf->Cell(195,5,iconv('UTF-8', 'ISO-8859-2', $txt),0,0,'C');	
$pdf->Ln(10);
$pdf->SetFont('Arial','',10);
$txt = 'En las instalaciones del Auditorio Centenario del Centro Recreacional COMFAMILIAR del Municipio de Puerto Asís, siendo las '.$hora.':'.$minuto.' del día '.$dia.' de '.$mes.' de '.$año.', certificamos los resultados del escrutinio efectuado para la elección de Revisor Fiscal '.$año.'-'.$siguienteaño;  
$pdf->MultiCell(0,5,iconv('UTF-8','',$txt));		
$pdf->Ln(20);
$pdf->SetFont('Arial','B',10);
$txt = 'REVISOR FISCAL '.$año.'-'.$siguienteaño;
$pdf->Cell(190,5,$txt,0,0,'C');
$pdf->Ln(5);
$pdf->SetFont('Arial','B',8);
$pdf->Cell(20,5,'ITEM',1,0,'C');
$pdf->Cell(97,5,'NOMBRES Y APELLIDOS',1,0,'C');
$pdf->Cell(73,5,iconv('UTF-8', 'ISO-8859-2', 'VOTACIÓN'),1,0,'C');
$pdf->Ln(5);
$cont = 1;
if($pregunta_items=pregunta_items($pregunta))
{
	...
	while($datos2= mysql_fetch_array($pregunta_items))
	{
		...
		{			
			...
			$pdf->Cell(97,5,iconv('UTF-8', 'ISO-8859-2', $datos2['item']),1,0,'L');
			$pdf->Cell(73,5,$respuestas,1,0,'C');
...
$pdf->Ln(5);
$pdf->Cell(117,5,iconv('UTF-8', 'ISO-8859-2','TOTAL VOTACIÓN'),1,0,'L');
$pdf->Cell(73,5,$acciones_votantes,1,0,'C');
$pdf->Ln(10);
...
$pdf->Cell(65,5,'',0,0,'L');
$pdf->Cell(65,5,iconv('UTF-8', 'ISO-8859-2','COMISIÓN ESCRUTADORA'),0,0,'L');
echo $pdf->Output("Acta_revisor_fiscal_".$año.".pdf",'D');
?>
